...
The [DCABM-TCM](http://bionet.ncpsb.org.cn/dcabm-tcm/) database [@liu2023DCABM] was used as the primary source to systematically retrieve blood‑absorbed compounds from each medicinal herb in ZWHQD, along with their chemical structural information. These candidate components were screened according to established criteria, and standardized chemical data were obtained from the [PubChem](https://pubchem.ncbi.nlm.nih.gov/) [@kim2025pubchem] database.
...
**Figure 1. Identification of disease‑associated candidate targets.**
(A) Volcano plot of DEGs (up/down counts labeled) Red dots represent significantly upregulated genes, blue dots represent significantly downregulated genes, and gray dots represent non-significant genes;
...
**Figure 2. Compound–disease target identification and enrichment.**
(A) Venn diagram of drug targets and disease candidate targets → core compound–disease targets. (B) Herb–compound–target network (node size by degree; targets from panel A). (C) GO enrichment faceted bar plot (top 10 biological process, cellular component, molecular function). (D) KEGG pathway enrichment bubble plot (top 20 pathways; gene count and adjusted p‑value).

**Figure 3. PPI network and hub gene screening.**
(A) PPI network colored by Degree (darker = higher connectivity). (B) UpSet plot of top 20 nodes from five CytoHubba algorithms (MCC, DMNC, MNC, Degree, EPC). (C) Most significant MCODE cluster network (score ≥ 5). (D) Venn diagram of CytoHubba consensus genes and MCODE cluster genes → hub genes.

**Figure 4.Immune infiltration landscape.**
...
**Figure 5. Machine learning‑based core diagnostic gene identification and nomogram.**
...
**Figure 6. Genetic validation and single‑cell enrichment of core genes.**
...
**Figure 7. Molecular docking of key compounds to core target proteins.**
...
**Figure 8. Proposed mechanistic model**
