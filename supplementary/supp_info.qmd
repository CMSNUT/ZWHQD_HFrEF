...
- **Figure S1. Quality assessment of sample expression profiles.**
(A) Distribution of expression density across samples.
(B) Boxplot of gene expression levels across samples.
(C) Principal component analysis (PCA) of sample expression profiles.
...
- **Table S4: GO and KEGG pathways enriched by core genes.**
  – Columns: Pathway_ID, Pathway_Description, Pathway_Type, Gene_Count, Core_Genes, p_value, p_adjust,
  
- **Table S5: Drug–Disease targets.**
  – Columns: Gene name, CytoHubba scores for four algorithms, MCODE cluster ID, Hub gene (Yes/No), Final core gene (Yes/No)
  
- **Table S6: Performance metrics of the diagnostic model based on core genes.**
