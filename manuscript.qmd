...
Subsequently, we retained high‑confidence target candidates using the following thresholds: score ≥0.84 for BATMAN‑TCM 2.0; probability ≥60% and accuracy ≥85% for SuperPred; probability > 0 for SwissTargetPrediction; z‑score >0 and Norm Fit >0.8 for PharmMapper; probability ≥0.5 for TargetNet; single human protein and Confidence Score >0.2 for PPB3; and P Value < 1e<sup>-3</sup>, Z.Score > 2 and  Max.Tc >0.3 for SEA. All predicted targets restricted to human origin only.
...
(E) Module-Module corraletion.
(F) Scatter plot of gene significance (GS) vs. module membership (MM) for key modules.
(G) Venn diagram showing the intersection of DEGs and WGCNA key module genes.
...
**Figure 5. Landscape of myocardial cell infiltration in normal and DCM-HFrEF samples.**
(A) Boxplots for intergroup abundance comparison of individual cell types; Wilcoxon test-derived *p*-values are marked, and extreme outliers were excluded.
(B) Percentage stacked bar graph for cellular composition ratios of two groups.
(C) Fold-change ranking of average cellular abundance (DCM-HFrEF/Normal), red dashed line = 1. (D) Upper-triangle Spearman correlation matrix among seven cell subtypes within DCM-HFrEF samples, numbers denote correlation coefficients.
(E) Spearman correlation between hub gene expression and infiltrated cell abundance in DCM-HFrEF myocardium. Red: positive correlation; blue: negative correlation; blank: non-significant (p≥0.05).
